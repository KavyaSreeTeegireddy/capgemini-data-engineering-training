...
Loaded CSV data into DataFrames and handled missing values using **dropna()**.Used **show()** to display results, referenced columns using **col()**, applied transformations like **groupBy()** with **sum()**, **avg()** and **count()**, sorted data using **orderBy()**, filtered using **filter()**, **isNull()** (to find customers with no sales), renamed columns using **alias()** and performed **joins** (inner and left) to combine customer and sales data.
## Key Learnings
