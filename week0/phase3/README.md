...
Loaded datasets into DataFrames and performed data cleaning using dropna(), filter(), and type casting with withColumn() + cast(). Combined datasets using join() (inner join), applied aggregations using groupBy() with sum(), avg(), and count(), used col(), alias(), and displayed outputs with show().
## Key Learnings
